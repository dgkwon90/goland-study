// direct exchange publish, consumer 테스트 소스 이다.
...
func StartConsumers() {
	...
	go func() {
		defer wg.Done()
		con1 := consumer.New(rabbitMqUrl, "consumer:1", exchangeName, "ucl", "user.created", nil)
		defer con1.Close()
		con1.Connection()
		con1.OpenChannel()
	...
	go func() {
		defer wg.Done()
		con2 := consumer.New(rabbitMqUrl, "consumer:2", exchangeName, "uul", "user.updated", nil)
		defer con2.Close()
		con2.Connection()
		con2.OpenChannel()
	...
	wg.Add(1)
	go func() {
		con3 := consumer.New(rabbitMqUrl, "consumer:3", exchangeName, "ucl.two", "user.created", nil)
		defer con3.Close()
		con3.Connection()
		con3.OpenChannel()
	...
	wg.Add(1)
	go func() {
		con4 := consumer.New(rabbitMqUrl, "consumer:4", exchangeName, "ucl", "user.created", nil)
		defer con4.Close()
		con4.Connection()
		con4.OpenChannel()
...
func main() {
	fmt.Println("============ [direct exchange test] ============")
